-- First, drop all constraints and indexes in correct order
DO $$ 
BEGIN
    -- Drop token constraints first
    ALTER TABLE IF EXISTS auth.one_time_tokens 
        DROP CONSTRAINT IF EXISTS one_time_tokens_pkey CASCADE;
    ALTER TABLE IF EXISTS auth.one_time_tokens 
        DROP CONSTRAINT IF EXISTS one_time_tokens_token_hash_key CASCADE;
    ALTER TABLE IF EXISTS auth.one_time_tokens 
        DROP CONSTRAINT IF EXISTS one_time_tokens_user_id_token_type_key CASCADE;
    ALTER TABLE IF EXISTS auth.one_time_tokens 
        DROP CONSTRAINT IF EXISTS one_time_tokens_user_id_fkey CASCADE;
        
    -- Drop other foreign key constraints
    ALTER TABLE IF EXISTS auth.identities 
        DROP CONSTRAINT IF EXISTS identities_user_id_fkey CASCADE;
    ALTER TABLE IF EXISTS auth.mfa_factors 
        DROP CONSTRAINT IF EXISTS mfa_factors_user_id_fkey CASCADE;
    ALTER TABLE IF EXISTS auth.sessions 
        DROP CONSTRAINT IF EXISTS sessions_user_id_fkey CASCADE;
    ALTER TABLE IF EXISTS auth.refresh_tokens
        DROP CONSTRAINT IF EXISTS refresh_tokens_session_id_fkey CASCADE;
    
    -- Drop unique constraints
    ALTER TABLE IF EXISTS auth.identities 
        DROP CONSTRAINT IF EXISTS identities_provider_id_provider_unique CASCADE;
    ALTER TABLE IF EXISTS auth.mfa_factors 
        DROP CONSTRAINT IF EXISTS unique_phone_factor_per_user CASCADE;
    ALTER TABLE IF EXISTS auth.mfa_amr_claims 
        DROP CONSTRAINT IF EXISTS mfa_amr_claims_session_id_authentication_method_pkey CASCADE;
    ALTER TABLE IF EXISTS auth.refresh_tokens 
        DROP CONSTRAINT IF EXISTS refresh_tokens_token_unique CASCADE;
    ALTER TABLE IF EXISTS auth.saml_providers 
        DROP CONSTRAINT IF EXISTS saml_providers_entity_id_key CASCADE;
END $$;
...
BEGIN
    EXECUTE (
        SELECT string_agg('DROP INDEX IF EXISTS ' || schemaname || '.' || indexname || ' CASCADE;', E'\n')
        FROM pg_indexes 
...
BEGIN
    EXECUTE (
        SELECT string_agg('DROP TYPE IF EXISTS ' || n.nspname || '.' || t.typname || ' CASCADE;', E'\n')
        FROM pg_type t 
